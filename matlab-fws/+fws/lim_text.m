function lim_text(ax)
%% fusion.get.lim - get lim
%
%    __           _             
%   / _|         (_)            
...
%------------- BEGIN CODE --------------
%
object_handles = findall(ax,'Type','text');
extent = zeros(numel(object_handles),4);
for ii=1:numel(object_handles)
    extent(ii,:) = object_handles(ii).Extent;
end

yPos = sum(extent(:,[2,4]),2);
xPos = sum(extent(:,[1,3]),2);
daspect(ax,[1,1,1]);
axis(ax,[min(extent(:,1)),max(xPos),min(extent(:,2)),max(yPos)])
%------------- END OF CODE --------------
