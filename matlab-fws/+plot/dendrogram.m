function [hout,T,perm,theGroups] = dendrogram(ax,Z,varargin)
...
if(~horz)
    for count = 1:(numLeaves-1)
        h(count) = line(ax,A(:,count),B(:,count),'color',col(count,:),'LineWidth',1.5);
    end
    lims = [0 numLeaves+1 max(0,ymin-margin) (ymax+margin)];
    set(ax, 'Xlim', [.5 ,(n +.5)], 'XTick', 1:n, 'XTickLabel', label, ...
        'Box', 'off');
    mask = logical([0 0 1 1]);
    if strcmp(orientation,'b')
        set(ax,'XAxisLocation','top','Ydir','reverse');
    end
else
    for count = 1:(numLeaves-1)
        h(count) = line(ax,B(:,count),A(:,count),'color',col(count,:),'LineWidth',1.5);
    end
    lims = [max(0,ymin-margin) (ymax+margin) 0 numLeaves+1 ];
    set(ax, 'Ylim', [.5 ,(n +.5)], 'YTick', 1:n, 'YTickLabel', label, ...
        'Box', 'off');
