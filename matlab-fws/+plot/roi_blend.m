function im = roi_blend(ax,obj,varargin)
% PLOT.ROI_BLEND: One line description of what the function or script performs
%
%   __           _             
%  / _|         (_)            
...
%  VERSION:  0.0 CREATED: 30-Aug-2020 19:33:31
%
% INPUTS:
%    input01 - 
%    input02 - 
...
% EXAMPLES:
%{
roi_blend(input01,input02,input03,input04)
%}
%
% DEPENDENCIES:
%
...
%------------- BEGIN CODE --------------
%
obj = struct('cortex',obj.cortex,'img',obj.im,'alpha',obj.alpha,...
...
imagesc(ax0,cat(3,m(mx,my)*bkg(1),m(mx,my)*bkg(2),m(mx,my)*bkg(3)),'AlphaData',imgaussfilt(m(mx,my),gf));
imagesc(ax1,img,'AlphaData',alpha);
imagesc(ax2,cortex,'AlphaData',AO.*m(mx,my)*0.5);
imagesc(ax3,Spec,'AlphaData',~(1-Spec).*m(mx,my));
